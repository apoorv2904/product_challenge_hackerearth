...
def preprocessor_input( x, **kwargs ):
    x = x -0.5
    x = x*2.0
    return(x)

def random_color_noise(x,rng=None, mu=0,sigma=0.1,  **kwargs):
    U = [[-0.50082839,  0.70805564,  0.4978234 ],
         [-0.58008777,  0.15228961, -0.80019126],
         [-0.64239327, -0.68953976,  0.33446346]]

    eigenvalues =  [ 0.88901423,  0.16066107,  0.07975197]
    U = np.asarray( U )
    eigenvalues = np.asarray( eigenvalues )
    # Generate the \alpha samples
    samples = rng.randn(3)*np.sqrt(sigma)

    augmentation = samples * eigenvalues
    noise = np.dot(U, augmentation.T)

    # Add the noise
    x = np.clip( x + noise, 0, 1)  # Scale here with the corresponding eigenvalue ?
    #x = x + noise  # Scale here with the corresponding eigenvalue ?
    return x
...
def random_brightness(x,rng=None, sig=0.5, **kwargs):
    #stds = np.random.randn(1) * std
    stds = rng.randn(1) * sig
    colour_channel_weights =  np.asarray( [ 0.88901423,  0.16066107,  0.07975197] )
    noise = stds[:, None] * colour_channel_weights[None, :]
    #x[:,:,0] = x + stds[0] * colour_channel_weights[0]
    #x[:,:,0] = x + stds[0] * colour_channel_weights[0]
    #x= np.clip(  np.reshape( stds[0] * colour_channel_weights, (1,1,3)) + x, 0, 1 )

    x = np.clip(x + noise[:, None, :], 0, 1)
    return x   
...
def center_crop_odd(x, center_crop_size, **kwargs):
    centerw, centerh = x.shape[0]//2, x.shape[1]//2
    halfw, halfh = center_crop_size[0]//2, center_crop_size[1]//2
...
class NumpyArrayIterator(Iterator):
    ...
    def __init__(self, X, y, image_data_generator,
        ...
        self.pool = pool
        self.length = image_data_generator.config['length']
	...
	#print( index_array )
	#print( self.image_data_generator.process )
